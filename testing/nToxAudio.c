...
char* help = "[i] commands:\n"
             "/f ID (to add friend)\n"
             "/m friendnumber message ( Send 'message' to 'friendnumber' )"
             "/c friendnumber ( Send audio call invite with 'friendnumber' )"
...
void line_eval ( char* line )
{
    if ( line[0] == '/' ) {
        ...
        if ( inpt_command == 'f' ) { // add friend command: /f ID
            ...
            get_id ( idstring );
            new_lines ( idstring );
        } else if ( inpt_command == 'c' ) { /* Initiate audio call. Format /c 0 */
            size_t len = strlen ( line );
            if ( len < 3 )
                return;
