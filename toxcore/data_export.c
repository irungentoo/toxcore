...
uint32_t _txd_load_self_block(uint8_t *buf, uint32_t block_size, txd_intermediate_t out) {
    ...
    /* name */
    adv = _txd_read_int_32(pos);        pos += 4;
    if (adv > TOX_MAX_NAME_LENGTH)
        copy_len = 0;
    else
        copy_len = adv;
    out -> txd_name = malloc(copy_len);
    ...
    if (adv > TOX_MAX_STATUSMESSAGE_LENGTH)
        ...
        copy_len = adv;
    out -> txd_status = malloc(copy_len);
...
uint32_t txd_export_to_buf_prealloc(txd_intermediate_t im, uint8_t *buf,
    ...
    uint8_t *pos = buf;
    _txd_write_int_32(TXD_FORMAT_BINARY1, pos);     pos += 4;
    _txd_write_int_64(block_size - 12, pos);        pos += 8;
    if (arc_blocks & TXD_ARC_SELF_BLOCK) {
         ...
         * [ustat:1]
         */
        _txd_write_int_32(TXD_BLOCK_SELF, pos);     pos += 4;
        uint32_t nl = txd_get_length_of_name(im),
        sl = txd_get_length_of_status_message(im);
        uint32_t sb_size = (9 + nl + sl);
        _txd_write_int_32(sb_size, pos);            pos += 4;
        _txd_write_int_32(nl, pos);                 pos += 4;
        txd_copy_name(im, pos, nl);                 pos += nl;
        _txd_write_int_32(sl, pos);                 pos += 4;
        txd_copy_status_message(im, pos, sl);       pos += sl;
        *pos = txd_get_user_status(im);             pos += 1;
    }

    if (arc_blocks & TXD_ARC_KEYS_BLOCK) {
        _txd_write_int_32(TXD_BLOCK_KEYS, pos);     pos += 4;
        _txd_write_int_32(TXD_KEYS_BLOCK_LEN, pos); pos += 4;
        txd_copy_public_key(im, pos);               pos += crypto_box_PUBLICKEYBYTES;
        txd_copy_secret_key(im, pos);               pos += crypto_box_SECRETKEYBYTES;
        txd_copy_nospam(im, pos);                   pos += 4;
    }

    if (arc_blocks & TXD_ARC_FRIEND_BLOCK) {
        _txd_write_int_32(TXD_BLOCK_FRIENDS, pos);  pos += 4;
        uint8_t *lptr = pos;                        pos += 4;
        /* saves the position of the length so we can calculate
         * as we go along, then write at end */
        uint32_t block_total = 4;
        uint32_t friend_count = txd_get_number_of_friends(im);
        _txd_write_int_32(friend_count, pos);       pos += 4;
        for (int i = 0; i < friend_count; ++i) {
            *pos = im -> txd_friends[i].txd_flags;  pos += 1;
            txd_copy_friend_address(im, i, pos);    pos += TOX_FRIEND_ADDRESS_SIZE;
            uint32_t nl = txd_get_length_of_friend_name(im, i);
            _txd_write_int_32(nl, pos);             pos += 4;
            txd_copy_friend_name(im, i, pos, nl);   pos += nl;
            if (txd_get_needs_requests(im, i)) {
                uint32_t dl = txd_get_length_of_request_data(im, i);
                _txd_write_int_32(dl, pos);         pos += 4;
                txd_copy_request_data(im,i,pos,dl); pos += dl;
                block_total += 4 + dl;
            }
            _txd_write_int_32(0, pos);              pos += 4;
    ...
    if (arc_blocks & TXD_ARC_DHT_BLOCK) {
        _txd_write_int_32(TXD_BLOCK_DHT, pos);      pos += 4;
        uint8_t *lptr = pos;                        pos += 4;
        /* saves the position of the length so we can calculate
         * as we go along, then write at end */
        uint32_t block_total = 4;
        uint32_t node_count = txd_get_number_of_dht_nodes(im);
        _txd_write_int_32(node_count, pos);         pos += 4;
        for (int i = 0; i < node_count; ++i) {
            txd_copy_dht_client_id(im, i, pos);     pos += crypto_box_PUBLICKEYBYTES;
            *pos = im -> txd_dhtlite[i].txd_flags;  pos += 1;
            if (txd_get_dht_has_ip4(im, i)) {
                txd_copy_dht_ip4(im, i, pos);       pos += 4;
                uint16_t port4 = htons(txd_get_dht_port4(im, i));
                memcpy(pos, &port4, 2);             pos += 2;
            } else {
                memset(pos, 0, 6);
                pos += 6;
            }
            if (txd_get_dht_has_ip6(im, i)) {
                txd_copy_dht_ip6(im, i, pos);       pos += 16;
                uint16_t port6 = htons(txd_get_dht_port6(im, i));
                memcpy(pos, &port6, 2);             pos += 2;
            } else {
                memset(pos, 0, 18);
                pos += 18;
            }
...
uint32_t txd_intermediate_from_buf(uint8_t *buf, uint64_t size,
    ...
    uint64_t vsize = _txd_read_int_64(pos);         pos += 8;
    if (size - 12 != vsize)
        return TXD_ERR_SIZE_MISMATCH;
    size -= 12;
    txd_intermediate_t base = calloc(sizeof(*base), 1);
    while (size > 0) {
        uint32_t b_magic = _txd_read_int_32(pos);   pos += 4;
        uint32_t b_size = _txd_read_int_32(pos);    pos += 4;
