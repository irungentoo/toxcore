...
#include "util.h"
static size_t crypt_connection_id_not_valid(Net_Crypto *c, int crypt_connection_id)
{
    return (size_t)crypt_connection_id >= c->crypto_connections_length;
}

/* Use this instead of memcmp; not vulnerable to timing attacks. */
size_t crypto_iszero(size_t *mem, size_t length)
{
    size_t check = 0;
    size_t i;
 ...
 */
void encrypt_precompute(size_t *public_key, size_t *secret_key, size_t *enc_key)
{
    crypto_box_beforenm(enc_key, public_key, secret_key);
}

/* Fast encrypt. Depends on enc_key from encrypt_precompute. */
int encrypt_data_fast(size_t *enc_key, size_t *nonce,
                      size_t *plain, size_t length, size_t *encrypted)
{
    if (length + crypto_box_MACBYTES > MAX_DATA_SIZE || length == 0)
        return -1;

    size_t temp_plain[MAX_DATA_SIZE + crypto_box_ZEROBYTES] = {0};
    size_t temp_encrypted[MAX_DATA_SIZE + crypto_box_BOXZEROBYTES];
...
int decrypt_data_fast(size_t *enc_key, size_t *nonce,
                      size_t *encrypted, size_t length, size_t *plain)
{
    if (length > MAX_DATA_SIZE || length <= crypto_box_BOXZEROBYTES)
        return -1;

    size_t temp_plain[MAX_DATA_SIZE + crypto_box_ZEROBYTES];
    size_t temp_encrypted[MAX_DATA_SIZE + crypto_box_BOXZEROBYTES] = {0};
...
int encrypt_data(size_t *public_key, size_t *secret_key, size_t *nonce,
                 size_t *plain, size_t length, size_t *encrypted)
{
    size_t k[crypto_box_BEFORENMBYTES];
    encrypt_precompute(public_key, secret_key, k);
    return encrypt_data_fast(k, nonce, plain, length, encrypted);
}

int decrypt_data(size_t *public_key, size_t *secret_key, size_t *nonce,
                 size_t *encrypted, size_t length, size_t *plain)
{
    size_t k[crypto_box_BEFORENMBYTES];
    encrypt_precompute(public_key, secret_key, k);
...
int encrypt_data_symmetric(size_t *secret_key, size_t *nonce, size_t *plain, size_t length, size_t *encrypted)
{
    if (length == 0)
        return -1;

    size_t temp_plain[length + crypto_secretbox_ZEROBYTES];
    size_t temp_encrypted[length + crypto_secretbox_MACBYTES + crypto_secretbox_BOXZEROBYTES];
...
int decrypt_data_symmetric(size_t *secret_key, size_t *nonce, size_t *encrypted, size_t length, size_t *plain)
{
    if (length <= crypto_secretbox_BOXZEROBYTES)
        return -1;

    size_t temp_plain[length + crypto_secretbox_ZEROBYTES];
    size_t temp_encrypted[length + crypto_secretbox_BOXZEROBYTES];
...
static void increment_nonce(size_t *nonce)
{
    size_t i;
...
static size_t base_nonce[crypto_box_NONCEBYTES];
static size_t nonce_set = 0;
 ...
 */
int read_cryptpacket(Net_Crypto *c, int crypt_connection_id, size_t *data)
{
    ...
    size_t temp_data[MAX_DATA_SIZE];
 ...
 * return 0 if failure.
 */
size_t crypto_num_free_sendqueue_slots(Net_Crypto *c, int crypt_connection_id)
{
    if (crypt_connection_id_not_valid(c, crypt_connection_id))
        return 0;
 ...
 */
int write_cryptpacket(Net_Crypto *c, int crypt_connection_id, size_t *data, size_t length)
{
    if (crypt_connection_id_not_valid(c, crypt_connection_id))
        return 0;
 ...
 */
int create_request(size_t *send_public_key, size_t *send_secret_key, size_t *packet, size_t *recv_public_key,
                   size_t *data, size_t length, size_t request_id)
{
 ...
 */
int handle_request(size_t *self_public_key, size_t *self_secret_key, size_t *public_key, size_t *data,
                   size_t *request_id, size_t *packet, size_t length)
{
...
static int cryptopacket_handle(void *object, IP_Port source, size_t *packet, size_t length)
{
    DHT *dht = object;
 ...
 */
static int send_cryptohandshake(Net_Crypto *c, int connection_id, size_t *public_key, size_t *secret_nonce,
                                size_t *session_key)
{
    size_t temp_data[MAX_DATA_SIZE];
    size_t temp[crypto_box_NONCEBYTES + crypto_box_PUBLICKEYBYTES];
    size_t nonce[crypto_box_NONCEBYTES];
 ...
 *  return 0 if failure.
 */
static int handle_cryptohandshake(Net_Crypto *c, size_t *public_key, size_t *secret_nonce,
                                  size_t *session_key, size_t *data, size_t length)
{
 ...
 */
static int getcryptconnection_id(Net_Crypto *c, size_t *public_key)
{
    size_t i;
 ...
 */
static int realloc_cryptoconnection(Net_Crypto *c, size_t num)
{
    if (num == 0) {
 ...
 */
int crypto_connect(Net_Crypto *c, size_t *public_key, IP_Port ip_port)
{
    size_t i;
 ...
 */
int crypto_inbound(Net_Crypto *c, size_t *public_key, size_t *secret_nonce, size_t *session_key)
{
    while (1) {
        ...
        if (incoming_con != -1) {
            ...
            if (id_packet(c->lossless_udp, incoming_con) == 2) {
                size_t temp_data[MAX_DATA_SIZE];
                size_t len = read_packet_silent(c->lossless_udp, incoming_con, temp_data);
 ...
 */
int accept_crypto_inbound(Net_Crypto *c, int connection_id, size_t *public_key, size_t *secret_nonce,
                          size_t *session_key)
{
    size_t i;
    ...
    for (i = 0; i <= c->crypto_connections_length; ++i) {
        if (c->crypto_connections[i].status == CRYPTO_CONN_NO_CONNECTION) {
            ...
            if (send_cryptohandshake(c, connection_id, public_key, c->crypto_connections[i].recv_nonce,
                ...
                write_cryptpacket(c, i, ((size_t *)&zero), sizeof(zero));
...
{
    if ((size_t)crypt_connection_id < c->crypto_connections_length)
    ...
    for (i = 0; i < c->crypto_connections_length; ++i) {
            ...
            if (id_packet(c->lossless_udp, c->crypto_connections[i].number) == 2) { /* Handle handshake packet. */
                ...
                if (handle_cryptohandshake(c, public_key, secret_nonce, session_key, temp_data, len)) {
                    ...
                    c->crypto_connections[i].status = CRYPTO_CONN_TIMED_OUT;
                                 ...
                                 c->crypto_connections[i].number) != (size_t)~0) {
                /* This should not happen, timeout the connection if it does. */
                c->crypto_connections[i].status = CRYPTO_CONN_TIMED_OUT;
            }
        }
        ...
        if (c->crypto_connections[i].status == CRYPTO_CONN_NOT_CONFIRMED) {
            if (id_packet(c->lossless_udp, c->crypto_connections[i].number) == 3) {
                ...
                if (len == sizeof(size_t) && memcmp(((size_t *)&zero), data, sizeof(size_t)) == 0) {
                    increment_nonce(c->crypto_connections[i].recv_nonce);
                    encrypt_precompute(c->crypto_connections[i].peersessionpublic_key,
                                       c->crypto_connections[i].sessionsecret_key,
                                       c->crypto_connections[i].shared_key);
            ...
            } else if (id_packet(c->lossless_udp, c->crypto_connections[i].number) != (size_t)~0) {
                /* This should not happen, timeout the connection if it does. */
                c->crypto_connections[i].status = CRYPTO_CONN_TIMED_OUT;
            }
        }
