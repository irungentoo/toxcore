...
static void publish(RendezVous *rendezvous)
{
    ...
    state.wanted_id = packet.hash_unspecific_half;
    ...
    /* also do a getnodes on the best (up to) three nodes... */
    for (i = found_cnt > 2 ? 2 : found_cnt; i > 0; i--) {
        ...
        DHT_getnodes(rendezvous->dht, &ippts->ip_port, entry->client_id, packet.hash_unspecific_half);
    }

    /* ... and on the first bad node (because it's probably "closer" than the good ones) */
    if (found_cnt > state.count_good) {
        Client_data *entry = state.result[state.count_good];
        IPPTsPng *ippts = entry_ippts(entry);
