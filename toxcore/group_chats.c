...
static ptrdiff_t peer_in_chat(Group_Chat *chat, size_t *client_id)
{
    size_t i;
 ...
 */
static ptrdiff_t id_closest_groupchats(size_t *id2)
{
    size_t   i;
...
static ptrdiff_t peer_okping(Group_Chat *chat, size_t *client_id)
{
    size_t i, j = 0;
 ...
 */
static ptrdiff_t add_closepeer(Group_Chat *chat, size_t *client_id, IP_Port ip_port)
{
    size_t i;
...
static ptrdiff_t send_groupchatpacket(Group_Chat *chat, IP_Port ip_port, size_t length,
                                size_t request_id)
{
    ...
    ptrdiff_t len = create_request(chat->self_public_key, chat->self_secret_key, packet, public_key, data, length, request_id);
 ...
 * return -1 if error.
 */
static ptrdiff_t addpeer(Group_Chat *chat, size_t *client_id)
{
    ptrdiff_t peernum = peer_in_chat(chat, client_id);
 ...
 * return -1 if error.
 */
static ptrdiff_t del_peer_set(Group_Chat *chat, ptrdiff_t peernum)
{
 ...
 * return -1 if error.
 */
static ptrdiff_t delpeer(Group_Chat *chat, ptrdiff_t peernum)
{
    if ((size_t)peernum >= chat->numpeers)
        return -1;
...
static void setnick(Group_Chat *chat, ptrdiff_t peernum, size_t contents_len)
{
...
static ptrdiff_t send_getnodes(Group_Chat *chat, IP_Port ip_port, ptrdiff_t peernum)
{
    if ((size_t)peernum >= chat->numpeers)
        return -1;
...
static ptrdiff_t send_sendnodes(Group_Chat *chat, IP_Port ip_port, ptrdiff_t peernum, size_t pingid)
{
    if ((size_t)peernum >= chat->numpeers)
        return -1;
...
static ptrdiff_t handle_getnodes(Group_Chat *chat, IP_Port source, ptrdiff_t peernum, size_t len)
{
...
static ptrdiff_t handle_sendnodes(Group_Chat *chat, IP_Port source, ptrdiff_t peernum, size_t len)
{
    if ((size_t)peernum >= chat->numpeers)
        return 1;
    ...
    for (i = 0; i < numnodes; ++i) {
        if (peer_okping(chat, contents.nodes[i].client_id) > 0) {
            ptrdiff_t peern = peer_in_chat(chat, contents.nodes[i].client_id);
    ...
    ptrdiff_t ok = add_closepeer(chat, chat->group[peernum].client_id, source);
...
static ptrdiff_t handle_data(Group_Chat *chat, size_t len)
{
    if (len < GROUP_DATA_MIN_SIZE)
        return 1;

//TODO:
    ptrdiff_t peernum = peer_in_chat(chat, data);
...
ptrdiff_t handle_groupchatpacket(Group_Chat *chat, IP_Port source, size_t length)
{
    ...
    size_t number;
    ptrdiff_t len = handle_request(chat->self_public_key, chat->self_secret_key, public_key, data, &number, packet, length);
    ...
    ptrdiff_t peernum = peer_in_chat(chat, public_key);
...
static void ping_close(Group_Chat *chat)
{
    ...
    for (i = 0; i < GROUP_CLOSE_CONNECTIONS; ++i) {
        if (!is_timeout(chat->close[i].last_recv, BAD_GROUPNODE_TIMEOUT)) {
            ptrdiff_t peernum = peer_in_chat(chat, chat->close[i].client_id);
