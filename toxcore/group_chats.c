...
#define GC_PLAIN_HS_REQ_PACKET_SIZE (HASH_ID_BYTES + sizeof(uint8_t))

#define GC_ENCRYPTED_HS_REQ_PACKET_SIZE (sizeof(uint8_t) + HASH_ID_BYTES + ENC_PUBLIC_KEY\
                                         + crypto_box_NONCEBYTES + GC_PLAIN_HS_REQ_PACKET_SIZE + crypto_box_MACBYTES)
...
enum {
    GH_INVITE_REQUEST,
    GH_PEER_INFO_EXCHANGE
} GROUP_HANDSHAKE_TYPE;
...
static void print_peer(const GC_GroupPeer *peer)
{
    ...
    fprintf(stderr, "Invite cert: %s\n", id_toa(peer->invite_certificate));  // Only prints first 32 bytes
 ...
 */
#define PACKED_GC_PEER_SIZE (EXT_PUBLIC_KEY + INVITE_CERT_SIGNED_SIZE + ROLE_CERT_SIGNED_SIZE\
...
static int pack_gc_peer(uint8_t *data, uint16_t length, const GC_GroupPeer *peer)
{
    ...
    memcpy(data + packed_length, peer->addr.public_key, EXT_PUBLIC_KEY);
...
static int unpack_gc_peer(GC_GroupPeer *peer, const uint8_t *data, uint16_t length)
{
    ...
    memcpy(peer->addr.public_key, data + len_processed, EXT_PUBLIC_KEY);
                                   ...
                                   uint8_t packet_type)
{
    ...
    int len = wrap_group_packet(chat->self_public_key, chat->group[peernumber].shared_key, packet, data,
...
static int handle_gc_sync_response(Messenger *m, int groupnumber, const uint8_t *public_key, uint32_t peernumber,
                                   const uint8_t *data, uint32_t length)
{
    ...
    for (i = 0; i < num_peers; i++) {
        if (peer_in_chat(chat, addrs[i].public_key) == -1)
            send_gc_handshake_request(m, groupnumber, addrs[i].ip_port, addrs[i].public_key, GH_PEER_INFO_EXCHANGE);
    }
...
static int handle_gc_sync_request(const Messenger *m, int groupnumber, const uint8_t *public_key,
                                  uint32_t peernumber, const uint8_t *data, uint32_t length)
{
    ...
    if (!ext_pk_equal(public_key, data)) {
        fprintf(stderr, "handle_gc_sync_request failed (ext_pk_equal failed)\n");
        return -1;
    }
    ...
    for (i = 0; i < chat->num_addrs; ++i) {
        if (peer_in_chat(chat, chat->addr_list[i].public_key) == -1)
            send_gc_handshake_request(c->messenger, chat->groupnumber, chat->addr_list[i].ip_port,
                                      chat->addr_list[i].public_key, GH_PEER_INFO_EXCHANGE);
    }
 ...
 * Return 0 if success
 */
static int gc_send_invite_request(GC_Chat *chat, uint32_t peernumber)
{
    uint8_t data[MAX_GC_PACKET_SIZE];
 ...
 * Return 0 if success
 */
static int handle_gc_invite_response(Messenger *m, int groupnumber, const uint8_t *sender_pk, uint32_t peernumber,
                                     const uint8_t *data, uint32_t length)
{
    GC_Chat *chat = gc_get_group(m->group_handler, groupnumber);
    ...
    if (!ext_pk_equal(sender_pk, data + SEMI_INVITE_CERT_SIGNED_SIZE)) {
    ...
    if (crypto_sign_verify_detached(data + INVITE_CERT_SIGNED_SIZE - SIGNATURE_SIZE, data,
                                    INVITE_CERT_SIGNED_SIZE - SIGNATURE_SIZE, SIG_PK(sender_pk)) != 0) {
 ...
 * Return 0 if success
 */
int handle_gc_invite_request(Messenger *m, int groupnumber, const uint8_t *public_key, uint32_t peernumber,
                             const uint8_t *data, uint32_t length)
{
    GC_Session *c = m->group_handler;
    GC_Chat *chat = gc_get_group(c, groupnumber);
...
static int handle_gc_peer_info_request(Messenger *m, int groupnumber, const uint8_t *sender_pk,
                                       uint32_t peernumber, const uint8_t *data, uint32_t length)
{
    if (!ext_pk_equal(sender_pk, data))
        return -1;
    ...
    return send_lossless_group_packet(chat, peernumber, packet, len, GP_PEER_INFO_RESPONSE);
}
...
static int send_gc_peer_info_request(GC_Chat *chat, uint32_t peernumber)
{
    ...
    memcpy(data, chat->self_public_key, EXT_PUBLIC_KEY);
    uint32_t len = EXT_PUBLIC_KEY;
    return send_lossless_group_packet(chat, peernumber, data, len, GP_PEER_INFO_REQUEST);
}
...
static int send_gc_peer_exchange(const GC_Session *c, GC_Chat *chat, uint32_t peernumber)
{
    ...
    if (peers_len <= 0) {
        fprintf(stderr, "pack_gc_peer failed in send_gc_peer_exchange %d\n", peers_len);
        return -1;
    }

    send_lossless_group_packet(chat, peernumber, data, len, GP_PEER_INFO_RESPONSE);
    send_gc_peer_info_request(chat, peernumber);
...
static int handle_gc_peer_info_response(Messenger *m, int groupnumber, const uint8_t *sender_pk,
                                        uint32_t peernumber, const uint8_t *data, uint32_t length)
{
    ...
    if (verify_cert_integrity(peer.invite_certificate) == -1) {
        fprintf(stderr, "handle_gc_peer_info_request fail! verify cert failed\n");
        return -1;
    }

    if (peer_update(m, groupnumber, &peer, peernumber) == -1) {
        fprintf(stderr, "peer_add() failed in handle_gc_peer_info_request\n");
        return -1;
    }

    memcpy(chat->group[peernumber].addr.public_key, sender_pk, EXT_PUBLIC_KEY);  // TODO: remove this
                                 ...
                                 uint32_t length)
{
    ...
    if (length > MAX_GC_NICK_SIZE || get_nick_peernumber(chat, nick, length) != -1) {
        ...
        fprintf(stderr, "wat\n");
        return send_gc_peer_info_request(chat, peernumber);
    }
 ...
 */
static int handle_gc_message_ack(GC_Chat *chat, const uint8_t *sender_pk, uint32_t peernumber,
                                 const uint8_t *data, uint32_t length)
{
    if (!ext_pk_equal(sender_pk, data))
        return -1;
...
static int handle_gc_broadcast(Messenger *m, int groupnumber, const uint8_t *sender_pk, uint32_t peernumber,
                               const uint8_t *data, uint32_t length)
{
    GC_Session *c = m->group_handler;
 ...
 * data must have room for GC_PLAIN_HS_REQ_PACKET_SIZE bytes.
 *
 * Returns length of plaintext data on success.
 * Returns -1 on failure.
 */
static int unwrap_handshake_request_packet(const uint8_t *self_sk, uint8_t *sender_pk, uint8_t *data,
                                           const uint8_t *packet, uint16_t length)
{
    ...
    int len = decrypt_data(sender_pk, self_sk, nonce,
                           ...
                           length - (1 + HASH_ID_BYTES + ENC_PUBLIC_KEY + crypto_box_NONCEBYTES), data);

    if (len != GC_PLAIN_HS_REQ_PACKET_SIZE) {
 ...
 * for GC_ENCRYPTED_HS_REQ_PACKET_SIZE bytes.
 *
 * Adds plaintext header consisting of: packet identifier, chat_id_hash, self public key, nonce.
 *
 * Returns length of encrypted packet on success.
 * Returns -1 on failure.
 */
static int wrap_handshake_request_packet(const uint8_t *self_pk, const uint8_t *shared_key, uint8_t *packet,
                                         const uint8_t *data, uint16_t length, uint32_t chat_id_hash)
{
    uint8_t nonce[crypto_box_NONCEBYTES];
    new_nonce(nonce);

    uint8_t encrypt[length + crypto_box_MACBYTES];
    int len = encrypt_data_symmetric(shared_key, nonce, data, length, encrypt);
    ...
    packet[0] = NET_PACKET_GC_HS_REQUEST;
    U32_to_bytes(packet + 1, chat_id_hash);
 ...
 *
 * Returns sent packet size on success.
 * Returns -1 on failure.
 */
static int send_gc_handshake_request(Messenger *m, int groupnumber, IP_Port ipp, const uint8_t *public_key,
                                     uint8_t hs_type)
{
    ...
    U32_to_bytes(data, chat->group[0].public_key_hash);
    memcpy(data + HASH_ID_BYTES, &hs_type, sizeof(uint8_t));
    uint32_t length = HASH_ID_BYTES + sizeof(uint8_t);
    uint8_t packet[GC_ENCRYPTED_HS_REQ_PACKET_SIZE];
    int enc_len = wrap_handshake_request_packet(chat->self_public_key, chat->group[peernumber].shared_key,
                                                packet, data, length, chat->chat_id_hash);
    if (enc_len != sizeof(packet)) {
        fprintf(stderr, "wrap_handshake_request_packet failed (len: %d\n)", enc_len);
        return -1;
    }

    return sendpacket(chat->net, ipp, packet, enc_len);
}

static int send_gc_handshake_response(GC_Chat *chat, uint32_t peernumber, uint8_t hs_type)
{
    uint8_t data[MAX_GC_PACKET_SIZE];
    memcpy(data, chat->self_public_key, EXT_PUBLIC_KEY);
    memcpy(data + EXT_PUBLIC_KEY, &hs_type, sizeof(uint8_t));
    uint32_t length = EXT_PUBLIC_KEY + sizeof(uint8_t);

    return send_lossless_group_packet(chat, peernumber, data, length, GP_HANDSHAKE_RESPONSE);
}

static int handle_gc_handshake_response(Messenger *m, int groupnumber, const uint8_t *sender_pk, uint32_t peernumber,
                                        const uint8_t *data, uint16_t length)
{
    ...
    uint8_t hs_type;
    memcpy(&hs_type, data + EXT_PUBLIC_KEY, sizeof(uint8_t));

    switch (hs_type) {
        case GH_INVITE_REQUEST:
            return gc_send_invite_request(chat, peernumber);
        case GH_PEER_INFO_EXCHANGE:
 ...
 * Return -1 on failure.
 */
int handle_gc_handshake_request_packet(void *object, IP_Port ipp, const uint8_t *packet, uint16_t length)
{
    if (length != GC_ENCRYPTED_HS_REQ_PACKET_SIZE)
        return -1;

    if (packet[0] != NET_PACKET_GC_HS_REQUEST)
        return -1;
    ...
    int len = unwrap_handshake_request_packet(chat->self_secret_key, sender_pk, data, packet, length);
    ...
    uint8_t hs_type;
    memcpy(&hs_type, data + HASH_ID_BYTES, sizeof(uint8_t));

    if (public_key_hash != jenkins_hash(sender_pk, ENC_PUBLIC_KEY))
        return -1;

    if (public_key_hash == chat->group[0].public_key_hash)
        return -1;

    int peer_exists = peer_in_chat(chat, sender_pk);

    if (peer_exists != -1)
        gc_peer_delete(m, chat->groupnumber, peer_exists, NULL, 0);

    int peernumber = peer_add(m, chat->groupnumber, &ipp, sender_pk);
    ...
    return send_gc_handshake_response(chat, peernumber, hs_type);
}
                              ...
                              uint8_t packet_type)
{
    ...
    switch (packet_type) {
        case GP_BROADCAST:
            ret = handle_gc_broadcast(m, groupnumber, sender_pk, peernumber, data, length);
            break;
        case GP_PEER_INFO_RESPONSE:
            ret = handle_gc_peer_info_response(m, groupnumber, sender_pk, peernumber, data, length);
            break;
        case GP_PEER_INFO_REQUEST:
            ret = handle_gc_peer_info_request(m, groupnumber, sender_pk, peernumber, data, length);
            break;
        case GP_SYNC_REQUEST:
            ret = handle_gc_sync_request(m, groupnumber, sender_pk, peernumber, data, length);
            break;
        case GP_SYNC_RESPONSE:
            ret = handle_gc_sync_response(m, groupnumber, sender_pk, peernumber, data, length);
            break;
        case GP_INVITE_REQUEST:
            ret = handle_gc_invite_request(m, groupnumber, sender_pk, peernumber, data, length);
            break;
        case GP_INVITE_RESPONSE:
            ret = handle_gc_invite_response(m, groupnumber, sender_pk, peernumber, data, length);
            break;
        case GP_HANDSHAKE_RESPONSE:
            ret = handle_gc_handshake_response(m, groupnumber, sender_pk, peernumber, data, length);
            break;
        default:
...
static int handle_gc_lossless_message(void *object, IP_Port ipp, const uint8_t *packet, uint16_t length)
{
    ...
    if (ext_pk_equal(chat->self_public_key, sender_pk))
        return -1;
    ...
    int len = unwrap_group_packet(chat->group[peernumber].shared_key, data, &message_id, &packet_type,
                                  packet, length);
    if (len <= 0)
        return -1;

    int lossless_ret = gcc_handle_recv_message(chat, peernumber, data, len, packet_type, message_id);
    ...
    int ret = handle_gc_lossless_helper(m, chat->groupnumber, ipp, sender_pk, peernumber, data, len,
                                        message_id, packet_type);
...
static int peer_update(Messenger *m, int groupnumber, GC_GroupPeer *peer, uint32_t peernumber)
{
    ...
    /* TODO: this state preserving shouldn't be necessary */
    peer->ignore = chat->group[peernumber].ignore;
    peer->time_connected = chat->group[peernumber].time_connected;
    peer->public_key_hash = chat->group[peernumber].public_key_hash;
    peer->last_rcvd_ping = chat->group[peernumber].last_rcvd_ping;
    peer->verified = chat->group[peernumber].verified;
    peer->confirmed = chat->group[peernumber].confirmed;
    ipport_copy(&peer->addr.ip_port, &chat->group[peernumber].addr.ip_port);
    memcpy(peer->addr.public_key, chat->group[peernumber].addr.public_key, EXT_PUBLIC_KEY);
    memcpy(peer->shared_key, chat->group[peernumber].shared_key, crypto_box_KEYBYTES);
...
static int peer_add(Messenger *m, int groupnumber, IP_Port *ipp, const uint8_t *public_key)
{
    ...
    encrypt_precompute(public_key, chat->self_secret_key, chat->group[peernumber].shared_key);
    memcpy(chat->group[peernumber].addr.public_key, public_key, ENC_PUBLIC_KEY);
    chat->group[peernumber].last_rcvd_ping = unix_time();
    chat->group[peernumber].time_connected = unix_time();
    chat->group[peernumber].public_key_hash = jenkins_hash(public_key, ENC_PUBLIC_KEY);
            ...
            case CS_DISCONNECTED: {
                if (chat->num_addrs && is_timeout(chat->last_join_attempt, GROUP_JOIN_ATTEMPT_INTERVAL)) {
                    send_gc_handshake_request(c->messenger, i, chat->addr_list[chat->addrs_idx].ip_port,
                                              chat->addr_list[chat->addrs_idx].public_key, GH_INVITE_REQUEST);
...
static int create_new_group(GC_Session *c, bool founder)
{
    ...
    chat->group[0].verified = founder ? true : false;
...
int gc_group_load(GC_Session *c, struct SAVED_GROUP *save)
{
    ...
    memcpy(chat->group[0].invite_certificate, save->self_invite_cert, INVITE_CERT_SIGNED_SIZE);
    ...
    chat->group[0].verified = (bool) save->self_verified;
...
GC_Session *new_groupchats(Messenger* m)
{
    ...
    networking_registerhandler(m->net, NET_PACKET_GC_HS_REQUEST, &handle_gc_handshake_request_packet, m);
...
void kill_groupchats(GC_Session *c)
{
    ...
    networking_registerhandler(c->messenger->net, NET_PACKET_GC_HS_REQUEST, NULL, NULL);
    free(c);
}
