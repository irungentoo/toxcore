...
static size_t id_distance(Assoc *assoc, void *callback_data, size_t *id_ref, size_t *id_test)
{
    ...
    while (bits > 8) {
        size_t distance = abs((int8_t)id_ref[pos] ^ (int8_t)id_test[pos]);
...
static size_t *dist_index_id(Assoc *assoc, size_t dist_ind)
{
...
static void dist_index_bubble(Assoc *assoc, size_t *dist_list, size_t first, size_t last, size_t *id,
...
static int assoc_id_closest(Assoc *assoc, void *callback_data, size_t *client_id, size_t *client_id1,
                            size_t *client_id2)
{
    return id_closest(client_id, client_id1, client_id2);
}

static bucket_t id_bucket(size_t *id, size_t bits)
{
...
static size_t candidates_search(Assoc *assoc, size_t *id, hash_t hash, Client_entry **entryptr)
{
...
static size_t candidates_create_internal(Assoc *assoc, hash_t hash, size_t *id, size_t seen,
        size_t used, bucket_t *bucketptr, size_t *posptr)
{
...
static size_t candidates_create_new(Assoc *assoc, hash_t hash, size_t *id, size_t used,
...
static size_t odd_min9_is_prime(size_t value)
{
    size_t i = 3;
...
#ifdef LOGGING
static char *idpart2str(size_t *id, size_t len);
#endif
