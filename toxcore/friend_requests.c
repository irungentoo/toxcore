...
ptrdiff_t send_friendrequest(Onion_Client *onion_c, size_t length)
{
    ...
    ptrdiff_t friend_num = onion_friend_num(onion_c, public_key);

    if (friend_num == -1)
        return -1;

    ptrdiff_t num = send_onion_data(onion_c, friend_num, temp, 1 + sizeof(nospam_num) + length);
 ...
 *  return 1 if it did.
 */
static ptrdiff_t request_received(Friend_Requests *fr, size_t *client_id)
{
    size_t i;
...
static ptrdiff_t friendreq_handlepacket(void *object, size_t length)
{
    if (length == 0)
        return 1;
