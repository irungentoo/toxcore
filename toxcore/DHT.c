...
int pack_ip_port(uint8_t *data, uint16_t length, uint16_t packed_length, const IP_Port *ip_port)
{
    ...
    if (ipv6 == 0) {
        uint32_t size = 1 + sizeof(IP4) + sizeof(uint16_t);

        if (packed_length + size > length)
            return -1;

        data[packed_length] = net_family;
        memcpy(data + packed_length + 1, &ip_port->ip.ip4, sizeof(IP4));
        memcpy(data + packed_length + 1 + sizeof(IP4), &ip_port->port, sizeof(uint16_t));
        return size;
    } else if (ipv6 == 1) {
        uint32_t size = 1 + sizeof(IP6) + sizeof(uint16_t);

        if (packed_length + size > length)
            return -1;

        data[packed_length] = net_family;
        memcpy(data + packed_length + 1, &ip_port->ip.ip6, sizeof(IP6));
        memcpy(data + packed_length + 1 + sizeof(IP6), &ip_port->port, sizeof(uint16_t));
        return size;
    } else {
        return -1;
    }
                   ...
                   uint8_t tcp_enabled)
{
    ...
    if (ipv6 == 0) {
        ...
        memcpy(&ip_port->ip.ip4, data + len_processed + 1, sizeof(IP4));
        memcpy(&ip_port->port, data + len_processed + 1 + sizeof(IP4), sizeof(uint16_t));
        return size;
    } else if (ipv6 == 1) {
        uint32_t size = 1 + sizeof(IP6) + sizeof(uint16_t);
        ...
        ip_port->ip.family = host_family;
        memcpy(&ip_port->ip.ip6, data + len_processed + 1, sizeof(IP6));
        memcpy(&ip_port->port, data + len_processed + 1 + sizeof(IP6), sizeof(uint16_t));
        return size;
    } else {
        return -1;
    }
