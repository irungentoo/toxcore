...
static const uint32_t jbuf_capacity = 3;
...
int toxav_kill_transmission ( ToxAv *av, int32_t call_index )
{
    ...
    rtp_kill(call->crtps[audio_index], av->messenger); call->crtps[audio_index] = NULL;
    rtp_kill(call->crtps[video_index], av->messenger); call->crtps[video_index] = NULL;
    cs_kill(call->cs); call->cs = NULL;
...
int toxav_recv_video ( ToxAv* av, int32_t call_index, vpx_image_t** out, uint16_t max_images, int32_t wait)
{
    ...
    return cs_reconstruct_video(av->calls[call_index].cs, out, max_images, wait);
}
...
int toxav_recv_audio(ToxAv* av, int32_t call_index, int16_t* dest, uint16_t max_size, int32_t wait)
{
    ...
    /* If CodecSession gets terminated while waiting this function returns -1 */
    return cs_reconstruct_audio(av->calls[call_index].cs, dest, max_size, wait);
}
