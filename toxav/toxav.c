        ...
        while ((pkt = vpx_codec_get_cx_data(call->video.second->encoder, &iter))) {
            if (pkt->kind == VPX_CODEC_CX_FRAME_PKT){
                ...
                if ((pkt->data.frame.flags & VPX_FRAME_IS_KEY) != 0 ){ /* is keyframe, send lossless. */
                    send_result = rtp_send_lossless_data(call->video.first, pkt->data.frame.buf, pkt->data.frame.sz);
                } else {
                    send_result = rtp_send_data(call->video.first, pkt->data.frame.buf, pkt->data.frame.sz);
                }
