...
int handle_recv_starting ( MSISession *session, MSICall* call, MSIMessage *msg )
{
    ...
    call->nonce_local = calloc ( sizeof ( uint8_t ), crypto_secretbox_NONCEBYTES );
...
int msi_answer ( MSISession* session, int32_t call_index, MSICallType call_type )
{
    ...
    session->calls[call_index]->nonce_local = calloc ( sizeof ( uint8_t ), crypto_secretbox_NONCEBYTES );
...
int msi_reject ( MSISession* session, int32_t call_index, const uint8_t* reason )
{
    ...
    send_message ( session, session->calls[call_index], _msg_reject, session->calls[call_index]->peers[session->calls[call_index]->peer_count - 1] );
