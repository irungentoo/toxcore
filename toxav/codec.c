...
int cs_reconstruct_video(CSSession* cs, vpx_image_t** dest, uint16_t max_images, int32_t wait)
{
    ...
    if (!cs->active) goto error; /* Inactive CodecState. Usual case when terminating cs. */
...
static int init_audio_decoder(CSSession *cs, uint32_t audio_channels)
{
    int rc;
...
int init_video_encoder(CSSession *cs, uint16_t max_width, uint16_t max_height, uint32_t video_bitrate)
{
...
int init_audio_encoder(CSSession *cs, uint32_t audio_channels)
{
    int rc = OPUS_OK;
...
CSSession *cs_new(const ToxAvCSettings *cs_self, const ToxAvCSettings *cs_peer, uint32_t jbuf_size, int video)
{
    ...
    if (pthread_mutex_init(cs->abuf_mutex, &attr) != 0) {
        pthread_mutexattr_destroy(&attr);
        goto error;
    }
    
    if (pthread_cond_init(cs->abuf_cond, NULL) != 0) {
        pthread_mutex_destroy(cs->abuf_mutex);
        pthread_mutexattr_destroy(&attr);
        goto error;
    }
    
    if ((cs->support_video = video)) {
        cs->max_video_frame_size = MAX_VIDEOFRAME_SIZE;
        cs->video_frame_piece_size = VIDEOFRAME_PIECE_SIZE;
        
        if (pthread_mutex_init(cs->vbuf_mutex, &attr) != 0) {
            pthread_mutexattr_destroy(&attr);
            goto error;
        }
        
        if (pthread_cond_init(cs->vbuf_cond, NULL) != 0) {
