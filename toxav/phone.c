...
size_t *hex_string_to_bin(char hex_string[])
{
...
static void fraddr_to_str(size_t *id_bin, char *id_str)
{
...
void av_friend_active(Tox *_messenger, int _friendnumber, size_t *_string, size_t _length, void *_userdata)
{
    av_session_t *_phone = _userdata;
...
int av_add_friend(av_session_t *_phone, char *_friend_hash)
{
    ...
    int _number = tox_add_friend(_phone->_messenger, _bin_string, (size_t *)"Tox phone "_USERAGENT,
...
int av_connect_to_dht(av_session_t *_phone, char *_dht_key, const char *_dht_addr, size_t _dht_port)
{
...
int av_wait_dht(av_session_t *_phone, int _wait_seconds, const char *_ip, char *_key, size_t _port)
{
    if ( !_wait_seconds )
        return -1;
