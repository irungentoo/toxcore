...
START_TEST(test_meetup)
{
    ...
    for (i = 0; i < 500; i++) {
        ...
        if (tox_isconnected(toxA) && tox_isconnected(toxB))
            break;
    }
    ...
    /* if we're just at the brink of an interval, wait a moment */
    uint64_t now_offset = now - now_floored;

    if (!now_offset || (now_offset >= RENDEZVOUS_INTERVAL - 2)) {
        /* wait until offset is +1 */
        for (;;) {
            unix_time_update();
            now = unix_time();
            now_floored = now - (now % RENDEZVOUS_INTERVAL);
            now_offset = now - now_floored;
